  ...
  <div class="uk-grid">
    <ul class="uk-width-1-2 uk-list uk-list-striped">
      <li>
        ...
        <a class="uk-link-text" href="#" @click="loadLocalFiles(getLocalDirUp)">..</a>
      </li>
      <li v-for="file in getSortedFiles" v-bind:key="file.path">
        ...
        <a class="uk-link-text" href="#" @click="loadLocalFiles(getLocalDirMove(file.path))" v-if="fileIsDirectory(file)">{{ file.path }}</a>
...
export default {
  name: 'FileManager',
  methods: {
    loadLocalFiles: function(url) {
      this.$store.dispatch('files/loadLocalFiles', { url });
    },
    getLocalDirMove(to) {
